...
PSI solve(string &str)
{
    VI suffixes=buildSuffixArray(str);
    VI LCP=buildLCP(suffixes, str);

    int maxLCP=LCP[0], index=0, counter=0;
    string repStr="";

    for(int i=1; i<SIZE(LCP) ; i++)
    {
        if(LCP[i]>maxLCP)
            maxLCP=LCP[i], index=i, counter=2, repStr=str.substr(suffixes[index], maxLCP);

        else if(LCP[i]==maxLCP && maxLCP!=0 && repStr==str.substr(suffixes[i], maxLCP))
            counter++;
    }

    return {str.substr(suffixes[index], maxLCP), counter};
}
...
int main()
{
    ...
    TEST_CASES(T)
    {
        ...
        PSI result=solve(str);

        result.SS ? cout<<result.FF<<" "<<result.SS<<"\n" : cout<<"No repetitions found!\n";
    }
